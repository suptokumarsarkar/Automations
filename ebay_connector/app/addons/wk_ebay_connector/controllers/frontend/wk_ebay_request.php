...
//    $tk = fn_wk_ebay_connector_createThumbnails(Registry::get('settings.Thumbnails'), '../httpdocs/images/detailed/' . $imageId . '/' . $file['name'], explode(".", $file['name'])[1], '/detailed/' . $imageId . '/' . $file['name']);
    ...
    foreach ($variants['Variation'] as $key => $variant) {
        ...
        $databases = db_get_fields("SELECT variant_id FROM ?:product_feature_variant_descriptions WHERE variant = '" . $variant['VariationSpecifics']['NameValueList'][0]['Value'] . "'");

        $featureId = db_get_row("SELECT feature_id FROM ?:product_features_descriptions WHERE internal_name = '" . $variant['VariationSpecifics']['NameValueList'][0]['Name'] . "'");
        $database = 0;
